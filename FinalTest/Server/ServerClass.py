...
class Server:
    ...
    def __init__(self, HOST, PORT):
        ...
        """Initialisation function to bind the server to a host and port."""
    ...
    def ao_direct_msg(self, msg_text, readable_socket):
        ...
        """Function called "ActionsOn_direct_msg" this further splits the DIRECT message into,
            different sub types.
        ...
        if parts[0] == "41":  # Chat initiation message
            ...
            if d.user_in_chatroom(sender_uname, parts[1])[0]:
                msg.send_msg(611, "{0}, has started a private chat.".format(sender_uname), readable_socket)    # Confirms to initiator
                msg.send_msg(614, "{0}, has started a private chat.".format(sender_uname), recipient_socket)    # Notifies the recipient
        ...
        elif parts[0] == "43":  # Decline chat invitation
            partner_uname = parts[1].split("-")[1]
            partner_sock = (next(iter({v for k, v in Server.user_socket_pairs.items() if k == partner_uname})))

            d.delete_chatroom(d.is_user_in_a_chatroom(partner_uname)[1][0][0])
            msg.send_msg(615, "{0} has declined a private chat.".format(partner_uname), partner_sock)
            return
        return
    ...
    def ao_command_msg(self, msg_text, readable_socket, msg_type):
        ...
        if msg_text[0] == "/":
            ...
            """This acts on the /ChatKick command sent from the client side, it checks if the
                user has admin rights. then if they do then the requested user is removed from
                the chatroom."""
