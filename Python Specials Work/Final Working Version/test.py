...
def player_setup():
    PLAYER_ONE_NAME = raw_input("PLAYER ONE: Enter your name...\t")
    message = PLAYER_ONE_NAME + " : Choose 'O' or 'X'\t"
    PLAYER_ONE_CHAR = raw_input(message).upper()
    ...
    if PLAYER_ONE_CHAR == 'X':
        ...
        PLAYER_ONE_CHAR = 'X'
        PLAYER_TWO_CHAR = 'O'
    PLAYER_TWO_NAME = raw_input("PLAYER TWO: Enter your name...\t")
...
def loop_until_move_valid(user):
    user_move = raw_input(user.name + ": Choose a position from 1 - 9...\t")
