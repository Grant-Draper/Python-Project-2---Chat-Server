...
class ServMessage:
    ...
    def send_msg(msg_type, msg_text, sock):
       ...
       #msg_text = msg_text[:-1]
        ...
        ServMessage.raw_send(sock, len(full_msg), full_msg)


    def receive_msg_from(sock):
        """This function waits for a message on a socket and returns the message type and text."""

        header = ServMessage.raw_receive(sock, HEADER_LENGTH)
        (msg_type, msg_length) = struct.unpack('!LL', header)

        try:
            msg_text = ServMessage.raw_receive(sock, msg_length).decode("utf-8")
